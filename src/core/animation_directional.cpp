...
{
  auto const& [size_x, size_y] = m_sprite_sheet->get_sprite_size().Get();
...
{
  t_str >> m_frame_start >> m_frame_end >> m_frame_row >> m_frame_time >> m_frame_action_start >> m_frame_action_end;
  if(t_str.fail()) {
    return false;
  }
  return true;
